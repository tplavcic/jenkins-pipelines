...
void makeReport(String GKEVer) {
    TestsReport = '<testsuite name=\\"PSMDB_GKE_${GKEVer}_${MDB_TAG}\\">\n'
...
void runTest(String TEST_NAME, String CLUSTER_PREFIX) {
    ...
    waitUntil {
        try {
            ...
            testsResultsMap["${params.GIT_BRANCH}-${env.GIT_SHORT_COMMIT}-$TEST_NAME-${params.GKE_VERSION}-$MDB_TAG"] = 'failure'
...
//                        conditionalRunTest('default-cr', 'basic')
    ...
    post {
        always {
            setTestsresults()
            makeReport("${params.GKE_VERSION}")
            script {
                TEST_RESULT_FILE_NAME="TestsReport_GKE_${params.GKE_VERSION}_${params.GIT_BRANCH}_${GIT_SHORT_COMMIT}_${MDB_TAG}.xml"
                sh """
            ...
            withCredentials([string(credentialsId: 'GCP_PROJECT_ID', variable: 'GCP_PROJECT'), file(credentialsId: 'gcloud-alpha-key-file', variable: 'CLIENT_SECRET_FILE')]) {
                sh '''
                    export CLUSTER_NAME=$(echo jenkins-param-psmdb-$(git -C source rev-parse --short HEAD) | tr '[:upper:]' '[:lower:]')
                    source $HOME/google-cloud-sdk/path.bash.inc
                    gcloud auth activate-service-account alpha-svc-acct@"${GCP_PROJECT}".iam.gserviceaccount.com --key-file=$CLIENT_SECRET_FILE
                    gcloud config set project $GCP_PROJECT
